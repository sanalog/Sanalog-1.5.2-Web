<%@ Control Language="C#" AutoEventWireup="true" CodeFile="articles-list-by-tag.ascx.cs" Inherits="modules_article_articles_list_by_tag" %>
<div class="module-title" <%= (this.baslikGorunsun ? "" : "style='display: none;'") %>><%= this.baslik %></div>

<p>
    <span class="alert alert-info tag-head">
        <asp:Literal ID="LtrMakaleTagAi" runat="server" />
        <%=GetGlobalResourceObject("default", "tag_list_head_text").ToString() %></span>
</p>
