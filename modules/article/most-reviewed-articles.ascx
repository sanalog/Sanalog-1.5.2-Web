<%@ Control Language="C#" AutoEventWireup="true" CodeFile="most-reviewed-articles.ascx.cs"
    Inherits="modules_makale_en_cok_yorumlanan_makaleler" %>
    ...
    <ul>
        <asp:Repeater runat="server" DataSourceID="SDSMak">
            <ItemTemplate>
                <li><a href='<%# Eval("SeoUrl", "/" + Snlg_ConfigValues.s_Dil + "/" + Snlg_ConfigValues.GetUrlValueByKey("blog") + "/{0}" + Snlg_ConfigValues.urlExtension)%>'><%# Eval("Baslik") %></a>
...
<asp:SqlDataSource ID="SDSMak" runat="server" ConnectionString="<%$ ConnectionStrings:e_cobiConn %>"
    SelectCommand="snlg_V1.zsp_Makale_EnCokYorumlanan" SelectCommandType="StoredProcedure"
