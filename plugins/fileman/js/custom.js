...
function FileSelected(file) {
    ...
    if (fieldId != "") {
        try { $(window.parent.document).find('#' + fieldId).attr('value', file.fullPath); } catch (e) { }
        try { $(window.parent.document).find('#' + fieldId).next('#sanalogFileImage').attr('src', file.fullPath); } catch (e) { }

        try { $(window.parent.document).find(".btnFMSave").click(); } catch (e) { }
        window.parent.SnlgOpenSingleManagerClose();
    }
}
