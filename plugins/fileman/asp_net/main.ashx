...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            _r.Write(GetErrorRes("Bu işlem için yetkili değilsiniz. sanalog-file-manager yetki anahtarı için yetki almalısınız."));
            return;
        }
        ...
        bool deger = true;
        vt.Ac();
        object sonuc = vt.ScalarSorguCalistir("snlg_V1.msp_AdminRolKontrol ", CommandType.StoredProcedure, new Snlg_DBParameter[2] { new Snlg_DBParameter("@pageName", SqlDbType.NVarChar, "sanalog-file-manager"), new Snlg_DBParameter("@UyeID", SqlDbType.Int, Snlg_MemberInfo.s_UserId) });
        ...
        if (!deger)
        {
            _r.Write(GetErrorRes("aBu işlem için yetkili değilsiniz. sanalog-file-manager yetki anahtarı için yetki almalısınız."));
            return;
        }
        #endregion
    ...
    private ArrayList ListDirs(string path)
    {
        ...
        if (path.EndsWith("\\Uploads"))
            dirs = new string[] { _context.Server.MapPath("\\Uploads\\Files"), _context.Server.MapPath("\\Uploads\\Images"), _context.Server.MapPath("\\Uploads\\Musics"), _context.Server.MapPath("\\Uploads\\Videos") };
        else
