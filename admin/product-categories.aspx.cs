...
public partial class admin_product_categories : Snlg_AdminBaseClass
{
    ...
    protected DataTable DTKtgler
    {
        get
        {
            if (_DTKtgler == null)
            {
                ...
                _DTKtgler = vt.DataTableOlustur("SELECT TMA.KtgId, TMA.UstId, TMA.Sira, TMA.Resim, TMA.Aktif, ISNULL(TMI.KtgAd, (SELECT TOP(1) KtgAd FROM snlg_V1.TblUrunKtgDetay WHERE KtgId = TMA.KtgId AND Dil = " + Snlg_ConfigValues.defaultLangId + ")) AS KtgAd, TMI.SeoUrl FROM snlg_V1.TblUrunKtgApp AS TMA LEFT OUTER JOIN snlg_V1.TblUrunKtgDetay AS TMI ON TMA.KtgId = TMI.KtgId AND TMI.Dil = " + dil, CommandType.Text);
                vt.Kapat();
